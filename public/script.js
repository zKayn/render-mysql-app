...
// Add cloud info section
function addCloudInfoSection() {
  if (!document.querySelector('.cloud-info')) {
    const cloudInfo = document.createElement('div');
    cloudInfo.className = 'cloud-info';
    cloudInfo.innerHTML = `
      <h3>Thông tin Cloud Computing</h3>
      <p>Ứng dụng này được triển khai trên nền tảng Cloud với các tính năng:</p>
      <ul>
        <li><strong>Auto Scaling:</strong> Tự động điều chỉnh số lượng instances dựa trên tải</li>
        <li><strong>Data Replication:</strong> Sao chép dữ liệu qua nhiều vùng địa lý</li>
        <li><strong>Load Balancing:</strong> Phân phối request đến các instances khác nhau</li>
        <li><strong>Cloud Monitoring:</strong> Giám sát hiệu suất ứng dụng</li>
      </ul>
      <p>Truy cập các tab trên thanh điều hướng để xem trực quan các tính năng.</p>
    `;
    
    // Add CSS style for cloud-info if not exists
    if (!document.querySelector('style#cloud-info-styles')) {
      const style = document.createElement('style');
      style.id = 'cloud-info-styles';
      style.textContent = `
        .cloud-info {
          margin-top: 30px;
          padding: 15px;
    ...
    container.appendChild(cloudInfo);
  }
}
